...
import { DELETED, DELETE_MEMBER } from "../../../utils/Constant";
...
const MembersTable = ({ data }) => {
  ...
  const handleDelete = (row) => {
    ...
    fetch("http://localhost:8000/members/" + row.id, {
      method: "DELETE",
      headers: {
      ...
      .then((res) => {
        ...
        fire(DELETE_MEMBER, DELETED);
        //Recalling the data from server
        dispatch(memberAsync()).unwrap();
      })
      .catch((err) => {
        console.log(err);
      });
